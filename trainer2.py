...
train_df['op22'] = train_df['op'].map({'+': 0, '-': 0, '/': 1, '*': 2}).astype(int)
#ja
train_df['op'] = train_df['op'].map({'+': operator.add, '-': operator.sub, '*': operator.mul, '/': operator.truediv})
#
...
for c in range(0, len(train_df)):
    ...
    #ja
    train_df['result'][c] = int(train_df.op[c](train_df.op1[c], train_df.op2[c]))
    #

    if train_df.op22[c] == 2: #mul
        #ja
        if train_df.op1[c] == 10 or train_df.op2[c] == 10 or train_df.op1[c] == 1 or train_df.op2[c] == 1:
            train_df['complexity'][c] = 1
        #
        elif train_df.op1[c] >9 and train_df.op2[c]>9:
            train_df['complexity'][c] = 3
        else:
            train_df['complexity'][c] = 2
    elif train_df.op22[c] == 1: #div
        if train_df.op1[c] > train_df.op2[c]:
            train_df['complexity'] = 1

    elif train_df.op22[c] == 0 and train_df.op1[c] > 9 and train_df.op2[c] > 9: #suma y resta
        train_df['complexity'][c] = 1
...
train_data = np.roll(train_data, 3) #si se meten mas varibles aumentar este numero
